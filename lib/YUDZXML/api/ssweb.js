...
async function ssweb(targetUrl, device = "phone", interval = 2000, maxRetries = 10) {
  ...
  throw new Error("Screenshot gagal dari API Apilight.");
}
...
// Handler API utama dengan fallback
module.exports = async (req, res) => {
  ...
  const handleRequest = async (imageUrl, device, fullPage, mime = "image/png") => {
    ...
    let result;
    try {
      // Kalau ada fullPage → coba fwss dulu
      ...
      try {
        ...
        throw new Error(`Kedua layanan gagal: ${e1.message} | ${e2.message}`);
      }
    }
